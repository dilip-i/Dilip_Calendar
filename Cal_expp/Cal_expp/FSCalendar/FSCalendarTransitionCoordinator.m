...
- (instancetype)initWithCalendar:(FSCalendar *)calendar
{
    ...
    if (self) {
        ...
        self.wCollectionView = self.calendar.wCollectionView;
        self.wCollectionViewLayout = self.calendar.wCollectionViewLayout;
    }
    return self;
}
...
- (void)scopeTransitionDidBegin:(UIPanGestureRecognizer *)panGesture
{
    ...
    if (self.transition == FSCalendarTransitionWeekToMonth) {
        ...
        self.wCollectionView.fs_top = -self.pendingAttributes.focusedRowNumber*self.calendar.wCollectionViewLayout.estimatedItemSize.height;
        ...
        case FSCalendarTransitionMonthToWeek: {
            ...
            if (animated) {
                ...
                if (self.calendar.delegate && ([self.calendar.delegate respondsToSelector:@selector(calendar:boundingRectWillChange:animated:)] || [self.calendar.delegate respondsToSelector:@selector(calendarCurrentScopeWillChange:animated:)])) {
                    ...
                    self.wCollectionView.fs_top = -attr.focusedRowNumber*self.calendar.wCollectionViewLayout.estimatedItemSize.height;
...
- (void)performTransitionCompletion:(FSCalendarTransition)transition animated:(BOOL)animated
{
    switch (transition) {
        case FSCalendarTransitionMonthToWeek: {
            ...
            [self.calendar.visibleWeekNoCells enumerateObjectsUsingBlock:^(UICollectionViewCell *obj, NSUInteger idx, BOOL * stop) {
                obj.contentView.layer.opacity = 1;
            }];
    ...
    switch (transition) {
        case FSCalendarTransitionMonthToWeek: {
            ...
            [self performAlphaAnimationFrom:MAX(1-progress*1.1,0) to:1 duration:0.3 exception:self.pendingAttributes.focusedRowNumber completion:^{
                ...
                [self.calendar.visibleWeekNoCells enumerateObjectsUsingBlock:^(__kindof UICollectionViewCell * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    obj.contentView.layer.opacity = 1;
                    [obj.contentView.layer removeAnimationForKey:@"opacity"];
                }];
                self.pendingAttributes = nil;
...
- (void)performPathAnimationWithProgress:(CGFloat)progress
{
    ...
    self.wCollectionView.fs_top = (-self.pendingAttributes.focusedRowNumber*self.calendar.wCollectionViewLayout.estimatedItemSize.height)*(self.transition == FSCalendarTransitionMonthToWeek?progress:(1-progress));
