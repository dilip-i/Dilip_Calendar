...
@interface FSCalendarWCollectionViewLayout ()
...
@implementation FSCalendarWCollectionViewLayout
...
- (void)prepareLayout
{
    ...
    self.estimatedItemSize = ({
        CGFloat width = (self.collectionView.fs_width-self.sectionInsets.left-self.sectionInsets.right)/1.0;
        CGFloat height = ({
        ...
        switch (self.scrollDirection) {
            ...
            case UICollectionViewScrollDirectionVertical: {
                ...
                for (NSInteger row = startRow; row <= endRow; row++) {
                    for (NSInteger column = 0; column < 1; column++) {
                        NSInteger section = 0;
                        NSInteger item = column + (row % 6) * 1;
                        NSIndexPath *indexPath = [NSIndexPath indexPathForItem:item inSection:section];
                        UICollectionViewLayoutAttributes *itemAttributes = [self layoutAttributesForItemAtIndexPath:indexPath];
                        [layoutAttributes addObject:itemAttributes];
